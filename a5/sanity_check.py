...
def question_1f_sanity_check():
    word_length = 8
    batch_size = 4
    output_expected_size = (batch_size, word_length)
    x_conv_test = torch.zeros(batch_size, word_length, dtype=torch.float)
    highway = Highway(word_length)
    x_highway_test = highway(x_conv_test)
    assert (x_highway_test.shape == output_expected_size) \
        , "output shape is incorrect: it should be:\n {} but is:\n{}".format(output_expected_size, x_highway_test.shape)
...
def question_1g_sanity_check():
    ...
    x_rshp_test = torch.zeros(batch_size, embed_size, max_word_length, dtype=torch.float)
    cnn = CNN(embed_size)
    x_cnn_test = cnn(x_rshp_test)
    assert (x_cnn_test.shape == output_expected_size) \
        , "output shape is incorrect: it should be:\n {} but is:\n{}".format(output_expected_size, x_cnn_test.shape)
    print("Sanity Check Passed for Question 1f: Highway Network!")
    print("-" * 80)
