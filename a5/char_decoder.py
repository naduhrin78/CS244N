...
class CharDecoder(nn.Module):
    ...
    def forward(self, input, dec_hidden=None):
        ...
        (h_t, dec_hidden) = self.charDecoder(x_emb, dec_hidden)
    ...
    def train_forward(self, char_sequence, dec_hidden=None):
        ...
        s_t, next_hidden = self.forward(char_sequence[:-1, :], dec_hidden)
        return nn.CrossEntropyLoss(reduction='sum', ignore_index=self.target_vocab.char_pad)(s_t.permute(1, 2, 0), char_sequence[1:, :].T)
        ### END YOUR CODE
    ...
    def decode_greedy(self, initialStates, device, max_length=21):
        ...
        sow = self.target_vocab.start_of_word
        eow = self.target_vocab.end_of_word
        dec_hidden = initialStates
        x_t = torch.tensor([[sow]*batch_size], device=device)
        for i in range(max_length):
            s_t, dec_hidden = self.forward(x_t, dec_hidden)
            x_t = s_t.argmax(dim=2)
            temp += x_t

        temp = torch.stack(temp).T.tolist()
        for line in temp:
            word = ""
            for w in line:
                if w == eow:
                    break
                word += self.target_vocab.id2char[w]
